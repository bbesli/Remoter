...
| `sftp_canonicalize` | `pane_id`, `path` | `String` |
| `sftp_read_link` | `pane_id`, `path` | `String` |
...
| Command | Takes | Returns |
|---|---|---|
| `sftp_enqueue` | `pane_id`, `Vec<TransferRequestDto>` | `Vec<TransferId>` |
...
is bounded and cancellable for the same reason a listing is.
...
**`TransferStatusDto` is flat, and carries what the transfer decided.**
`transferId`, `direction`, `remote`, `remoteDisplay`, `local`, `resume`,
`state`, and a `start` block holding `resumeRequested`, `resumeFrom`, `total`,
`resumeDeclined` and a `note`. The note is the sentence this document asks for —
...
part of the SFTP command surface.
