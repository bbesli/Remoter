...
> built yet: no session recording, no FIDO2 key slot, no FTP, no plugin host, and
> the clipboard carries text over RDP only. [What works, and what does not](#what-works-today) is
...
| **SSH** | A terminal that copies, pastes and clicks the way the platform's own does — Windows Terminal, Terminal.app, GNOME Terminal — including the Linux PRIMARY selection ([ADR-0015](docs/architecture/decisions/0015-terminal-follows-the-platform.md)); PTY shell and `exec`, public key / password / keyboard-interactive, with a key passphrase, a 2FA code or an expired password typed into the tab's own dialog when nothing stored answers it (never saved), host key TOFU and pinning, agent auth and agent forwarding (both off by default), environment variables, initial command, compression, keep-alive | GSSAPI/Kerberos, SSH certificates, X11 forwarding, per-connection algorithm preferences |
...
| **Clipboard** | RDP text both ways over MS-RDPECLIP. Copy on the server and it is on this machine's clipboard; copy here and it is offered to the server when the tab takes the keyboard and again before Ctrl+V. Text the server already has is never offered back, so a rich copy on the server keeps its formatting. Each direction is a setting on the connection. A copy too large to carry, or a server whose policy forbids the clipboard, is said on the tab rather than ending the session | Files and images over the clipboard. VNC in either direction — it can write the remote clipboard on the wire, but the interface offers VNC no clipboard yet. SSH has none of its own: the terminal copies and pastes locally |
