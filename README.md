...
| **Import** | Remoter's own `.rmtr` archive with its passwords and keys; mRemoteNG `confCons.xml` (GCM and legacy CBC), `~/.ssh/config`, CSV — each with detection, preview, findings report and an all-or-nothing commit | Royal TS, PuTTY, RDCMan, `.rdp`; `known_hosts` into the trust store; JSON |
| **Export** | The whole vault or one folder, for **another Remoter** as an encrypted `.rmtr` archive with the server passwords and keys — sealed under a password you set, bringing along the shared credentials and jump hosts the folder uses — or for **other tools** as CSV (one row per connection, inherited values written in), an OpenSSH config (SSH and SFTP connections as `Host` blocks, routes as `ProxyJump`) or JSON (the tree as stored). The flat formats never carry a secret and say what they could not carry; the archive, CSV and OpenSSH config read back through their importers; recorded in the audit log | Plaintext secret export, importing the JSON back |
