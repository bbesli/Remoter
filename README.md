...
| **Import** | Remoter's own `.rmtr` archive with its passwords and keys, and its JSON export; mRemoteNG `confCons.xml` (GCM and legacy CBC), `~/.ssh/config`, CSV — each with detection, preview, findings report, a choice about items the vault already has (keep both, skip or replace; existing folders are merged into) and an all-or-nothing commit | Royal TS, PuTTY, RDCMan, `.rdp`; `known_hosts` into the trust store; a conflict choice per item |
